...
class ManoLayer(Module):
    ...
    def forward(self, th_pose_coeffs, th_betas=torch.zeros(1), th_trans=None):
        """
        Args:
        ...
        if self.use_pca or self.joint_rot_mode == "axisang":
            ...
            th_full_pose = torch.cat(
                [th_pose_coeffs[:, : self.rot], self.th_hands_mean + th_full_hand_pose],
                1,
            )
        ...
        if th_betas is None or th_betas.numel() == 1:
            th_v_shaped = (
                torch.matmul(self.th_shapedirs, self.th_betas.transpose(1, 0)).permute(
                    2, 0, 1
                )
                + self.th_v_template
            )
            ...
            th_v_shaped = (
                torch.matmul(self.th_shapedirs, th_betas.transpose(1, 0)).permute(
                    2, 0, 1
                )
                + self.th_v_template
            )
